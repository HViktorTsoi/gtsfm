...
class SfmTrack2d(NamedTuple):
    ...
    def generate_tracks_from_pairwise_matches(
        ...
        Returns:
            ...
            Percent of tracks rejected due to multiple features from the same image being present.
        """
        ...
        erroneous_track_pct = erroneous_track_count / len(key_set)
        logger.info(
            f"DSF Union-Find: {erroneous_track_pct*100:.2f}% of tracks discarded from multiple obs. in a single image."
        )
        return track_2d_list, erroneous_track_pct
